resource "null_resource" "get_nlb_hostname" {
  provisioner "local-exec" {
    command = "aws eks update-kubeconfig --name ${var.project_name}-${var.dept}-${var.env} --region ${var.region} && kubectl get svc load-nginx  --namespace nginx-ingress -o jsonpath='{.status.loadBalancer.ingress[*].hostname}' > ${path.module}/lb_hostname.txt"
  }
  depends_on = [
...
locals {
  instances = {
    namea = "sockshop.${var.domain_name}"
